...
describe("Database - gestione animali", function () {
    ...
    test("Popolazione database", async function () {
        ...
        tmp = await new UserModel({ username: "user1", password: "a", email: "user1@test.it", name: "User1", surname: "Uno" }).save();
        users_id.push(tmp._id);
        tmp = await new UserModel({ username: "user2", password: "a", email: "user2@test.it", name: "User2", surname: "Due" }).save();
        users_id.push(tmp._id);
        tmp = await new UserModel({ username: "user3", password: "a", email: "user3@test.it", name: "User3", surname: "Tre" }).save();
        users_id.push(tmp._id);
    });

    test("Verifica duplicati", async function () {
        const animal_id = randomOf(animals_id);
        await UserModel.findByIdAndUpdate(users_id[0], { $addToSet: { animals_id: animal_id } });
        await UserModel.findByIdAndUpdate(users_id[0], { $addToSet: { animals_id: animal_id } });

        const user = await UserModel.findById(users_id[0]).exec();
